...
export default function MapPage() {
  ...
  // When the page is opened with ?store=N, auto-select that store once data lands.
  useEffect(() => {
    if (!highlightStoreId || !filteredNearbyStores) return;
    const target = filteredNearbyStores.find(
      (s) => s.id === parseInt(highlightStoreId)
    );
    if (!target) return;
    setSelectedStore({
      id: target.id,
  ...
  }, [highlightStoreId, filteredNearbyStores]);
