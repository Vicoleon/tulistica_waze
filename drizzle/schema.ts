...
export const integrationCredentials = mysqlTable("integration_credentials", {
  id: int("id").autoincrement().primaryKey(),
  // App-wide credentials use a null userId. Per-user credentials carry a user id.
  userId: int("userId"),
  // Identifier for the integration: 'automercado', etc.
  integration: varchar("integration", { length: 64 }).notNull(),
  // Optional display label so users can distinguish multiple accounts.
  label: varchar("label", { length: 128 }),
  // Encrypted JSON blob — never read directly, always go through vault helper.
  ciphertext: text("ciphertext").notNull(),
  // When this credential was last successfully verified by the scraper.
  lastVerifiedAt: timestamp("lastVerifiedAt"),
  // Last error encountered when using these credentials, for ops debugging.
