 ...
 * Uses Google Maps Places API via Manus proxy (makeRequest helper)
 */
...
async function searchNearbyStores(lat: number, lng: number, radius = 5000) {
  try {
    // Use the makeRequest helper with correct endpoint format
...
async function seedStores() {
  ...
  const allStores = new Map<string, {
    placeId: string;
    name: string;
    address: string;
    latitude: number;
  ...
  for (const [placeId, store] of allStores) {
    try {
      ...
      const storeData = {
        name: store.name,
        chainId: detectChain(store.name),
