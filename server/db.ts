 ...
 * break a user-facing request. Denormalizes `tier`, `cadence`, `householdSize`
 * from the user's shopper profile at insert time so dashboards can aggregate
 * without JOINs.
 */
export async function recordAnalyticsEvent(
  ...
  try {
    ...
    // Swallow — never break the request because analytics is down.
    console.warn("[Analytics] Failed to record event:", error);
  }
}

/**
 * Aggregations powering the /admin/analytics dashboard. Each returns shapes
 * tailored for the UI (no further transformation needed on the client).
 */

/**
 * mysql2 returns `[rows, fields]` from execute(). Normalize to just the rows
 * array regardless of how drizzle wraps it.
 */
function unwrapRows<T>(result: unknown): T[] {
  if (Array.isArray(result)) {
    const first = (result as unknown[])[0];
    if (Array.isArray(first)) return first as T[];
    // Drizzle may already unwrap to a flat array of objects.
...
export async function getAnalyticsSummary(days = 7) {
  ...
  // Users by tier — snapshot (no time window).
...
export async function getBrandByEmail(
  ...
  const rows = await db
    .select()
    .from(brands)
    .where(eq(brands.ownerEmail, email.toLowerCase()))
    .limit(1);
  return rows[0];
}
...
export async function createBrand(input: InsertBrand): Promise<number | null> {
  const db = await getDb();
  if (!db) return null;
...
export async function getCampaignsForBrand(brandId: number) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
...
export async function createCampaignForBrand(
  brandId: number,
  input: Omit<InsertAdCampaign, "brandId">
): Promise<number | null> {
  const db = await getDb();
  if (!db) return null;
  const result = await db.insert(adCampaigns).values({ ...input, brandId });
  return result[0].insertId;
}
...
export async function updateCampaign(
  ...
  const db = await getDb();
  if (!db) return;
  // Defensive: only allow updating campaigns owned by this brand.
  await db
    .update(adCampaigns)
    .set(patch)
    .where(and(eq(adCampaigns.id, id), eq(adCampaigns.brandId, brandId)));
}
